...
(define (or-args exp) (rest exp))
(define (make-or args) (cons 'or args))
;;;;;;;;;;;;;;;;; Конец  вставки ;;;;;;;;;;;;;;;;;
...
;; a
;; > (or->if '(or a1 a2 a3))
;; (let ([v1 a1]) (if v1 v1 (let ([v2 a2]) (if v2 v2 a3))))

;; Более простым переводом (or a1 a2 a3) является (if a1 a1 (if a2 a2 a3)),
;; однако он неправильный, если a1 или a2 используют побочные эффекты,
;; так как в переводе a1 и a2 используются по два раза, в то время как
;; в исходном выражении они используются по одному разу.

;; Переменные v1 и v2 не должны встречаться в a1, a2 и a3. Для генериррвания
;; имен этих переменных следует использовать функцию (gensym s), которая
;; возвращает символ, полученный конкатенацией символа s и случайного числа.
;; Например, (gensym 'abc) может вернуть abc69544.

;; И or, и if рассматривают константу #f как ложь, а все остальные
;; значения как истину.
...
;; #t
;; а не ошибка!
;; > (meval '(or (> 0 1) (car '(#f 3)) #f))
;; #f
;; > (meval '(or (begin (displayln 1) 2) 3))
;; 1
;; 2
...
(define (or->if exp)
  (define (go args)
    (cond
      ...
      [else (let* ([rst (go (rest args))]
                   [val (gensym 'val)]
                   [cnd (make-if val val rst)])
              (make-let (list (list val (first args))) (list cnd)))]))
  (go (or-args exp)))
