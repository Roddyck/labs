...
;;;;;;;;;;;;;;;;; Начало вставки ;;;;;;;;;;;;;;;;;
; (for (i lst) body1 body2 ...)

(define (for? exp) (tagged-list? exp 'for))
(define (for-var exp) (caadr exp))
(define (for-range exp) (cadadr exp))
(define (for-body exp) (cddr exp)) ; возвращает список выражений
;;;;;;;;;;;;;;;;; Конец вставки ;;;;;;;;;;;;;;;;;
...
(define (m-eval exp env)
  (cond [(self-evaluating? exp) exp]
        ...
        ;; ((for? exp) (m-eval (transform-for3 exp) env))
        ;;;;;;;;;;;;;;;;; Конец вставки ;;;;;;;;;;;;;;;;;
        [(application? exp)
...
; Добавление for в ядро интерпретатора

; (for (i lst) body1 body2 ...)
; Для каждого элемента i списка lst вычислить
; выражения body1, body2, ...
...
; Здесь первые три строчки печаются displayln, а последнее из
; возвращаемое значение.

(define (eval-for exp env)
  (let ([var (for-var exp)]
        [rng-exp (for-range exp)]
        [body (for-body exp)])
    (let ([rng (m-eval rng-exp env)]
          [env1 (extend-environment (make-frame var #f) env)])
      (define (loop lst)
        (cond
          [(null? lst) (void)]
          [else (set-variable-value! var (car lst) env1)
                (eval-sequence body env1)
                (loop (cdr lst))]))
      (loop rng))))

;;;;;;;;;;;;;;;;; Преобразование for в рекурсию ;;;;;;;;;;;;;;;;;

; for-test1 можно эквивалентно записать следующим образом.
...
; В общем случае выражение (for (i lst-expr) body1 body2 ...)
...
; Пустой let нужен, так как особая форма define может
; быть расположена только в определенных местах:
; внутри let, lambda, define.

; Описание квазикавычек (quasiquote) см. по ссылке Solutions
; под Lecture 4 на сайте курса MIT https://web.mit.edu/6.001/6.037/.
; или в The Racket Guide на docs.racket-lang.org.

; Пример использования квазикавычек (функция snoc дальше не используется).

(define (snoc lst x) `(,@lst ,x))

; Первый вариант функции transform-for, преобразующий for в рекурсию.

(define (transform-for1 exp)
  (let ([i (for-var exp)]
        [lst-exp (for-range exp)]
        [body (for-body exp)])
    `(let ()
       (define (go lst)
         (if (null? lst)
...
; Следующий тест (meval for-test3) вызывает ошибку, если в m-eval
; используется transform-for1.
...
; Действительно, в transform-for1 может возникнуть коллизия
; переменных, если в body используются переменные go или lst. Функция
; (gensym "str") возвращает символ, который начинается на str и
; оканчивается на случайное число. Она используется для создания
; уникальных идентификаторов.

(define (transform-for2 exp)
  (let ([i (for-var exp)]
        [lst-exp (for-range exp)]
        [body (for-body exp)]
        [go (gensym "go")] ; на случай, если переменная go используется в body
        [lst (gensym "lst")])
    `(let ()
       (define (,go ,lst)
         (if (null? ,lst)
...
; Недостаток transform-for2 состоит в том, что он использует конкретный
; синтаксис. Ниже находится вариант 3, использующий функции из части 1
; для создания особых форм.

(define (transform-for3 exp)
  (let ([i (for-var exp)]
        [lst-exp (for-range exp)]
        [body (for-body exp)]
        [go (gensym "go")]
    ...
    (make-let '()
      (list
        (make-define-func go (list lst)
          (list (make-if `(null? ,lst)
                         '(void)
                          (make-let `([,i (car ,lst)])
                           `(,@body
                             (,go (cdr ,lst)))))))
...
;; Добавьте в ядро интерпретатора особую форму for с несколькими
...
;; Во время цикла переменные параллельно продвигаются каждая по своему
;; списку. Цикл заканчивается, когда заканчивается самый короткий список.
;; Значение каждого выражения в теле цикла игнорируется, и значением
;; всего выражения является #<void>.
