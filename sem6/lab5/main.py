...
def diff_scheme2(step: float, y0: float, y1: float, x0: float) -> float:
    return (y1 * (2/step**2 + 2*x0**2) + y0 * (-1/step**2 + (x0+1)/2*step)
            - 2*x0**5 + 3 * x0**3 + 6*x0 + x0**2) / (1/step**2 + (x0+1)/2*step)

def true_sol2(x: float) -> float:
    return x**3 + 1

def diff_equation_solve(scheme: Callable[[float, float, float, float], float],
                        y0: float, y1: float, x0: float, num_iters: int,
...
def problem_solve(scheme: Callable[[float, float, float, float], float],
                  y0: tuple[float, float], y1: tuple[float, float], x0: float,
                  num_iters: int, step: float, beta: float,
                  true_sol: Callable[[float], float]) -> tuple[PrettyTable, list[float], list[float], list[float], float]:
    ...
    mu = (beta - second_problem_itrs[-1]) / (first_problem_itrs[-1] - second_problem_itrs[-1])
    ...
    for i, (y_first, y_second, y_sol) in enumerate(zip(first_problem_itrs, second_problem_itrs, solution)):
        true_value = true_sol(x_0)
        abs_error = abs(y_sol - true_value)
        mean_error += abs_error

        table.add_row([i, round(x_0, 5), round(y_first, 5), round(y_second, 5), round(y_sol, 5),
              round(true_value, 5), round(abs_error, 10)])
        x_0 += step
...
def show_plots(sol: Callable[[float], float], x0: float, x1: float,
               itrs1: list[float], itrs2: list[float],
               itrs_sol: list[float]) -> None:
...
def main():
    def part1():
        ...
        table, itrs1, itrs2, sol, _ = problem_solve(diff_scheme1, y0, y1, x0, n, h, beta, true_sol1)
        print(table)
    ...
    def part2():
        ...
        table, itrs1, itrs2, sol, _ = problem_solve(diff_scheme2, y0, y1, x0, n, h, beta, true_sol2)
        print(table)
    ...
    def part3():
        ...
        while n <= 80:
            ...
            table, _, _, _, mean_error = problem_solve(diff_scheme1, y0, y1, x0, n, h, beta, true_sol1)
